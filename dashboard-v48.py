...
def build_clean_dzen_url(channel_url, dzen_id=""):
    url_str = str(channel_url or "").strip()
    id_str = str(dzen_id or "").strip()
    
    if not url_str or url_str.lower() in ["none", "nan", "null", ""]:
    ...
    if "dzen.guru" in url_str:
        ...
        return f"https://dzen.ru/{clean_path}"
        
    if not url_str.startswith("http://") and not url_str.startswith("https://"):
        if url_str.startswith("dzen.ru") or url_str.startswith("www.dzen.ru"):
            return f"https://{url_str}"
        return f"https://dzen.ru/{url_str.lstrip('/')}"
